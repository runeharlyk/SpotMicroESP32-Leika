...
class AnalyticsService {
  ...
  private:
    ...
    void updateAnalytics() {
        ...
        doc["cpu0_usage"] = taskManager.getCpuUsage(0);
        doc["cpu1_usage"] = taskManager.getCpuUsage(1);
        doc["cpu_usage"] = taskManager.getCpuUsage();
        JsonArray tasks = doc["tasks"].to<JsonArray>();
        for (auto const &task : taskManager.getTasks()) {
