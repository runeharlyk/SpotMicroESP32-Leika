...
class RuntimeShape {
  public:
    ...
    RuntimeShape(const std::initializer_list<int> init_list) : size_(0) { BuildFrom(init_list); }
    ...
    inline int32_t* DimsData() { return size_ > kMaxSmallSize ? dims_pointer_ : dims_; }
    inline const int32_t* DimsData() const { return size_ > kMaxSmallSize ? dims_pointer_ : dims_; }
  ...
  private:
    ...
    RuntimeShape(int new_shape_size, const RuntimeShape& shape, int pad_value) : size_(0) {
...
inline tflite::Dims<4> ToRuntimeDims(const tflite::RuntimeShape& array_shape) {
    ...
    int cum_prod = 1;
    for (int i = 0; i < 4; i++) {
        const int new_dim = (i < dimensions_count) ? array_shape.Dims(dimensions_count - 1 - i) : 1;
...
inline RuntimeShape DimsToShape(const tflite::Dims<4>& dims) {
    return RuntimeShape({dims.sizes[3], dims.sizes[2], dims.sizes[1], dims.sizes[0]});
}
