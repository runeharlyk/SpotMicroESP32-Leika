...
inline void Mean(const tflite::MeanParams& op_params, int32_t multiplier, int32_t shift,
                 const RuntimeShape& unextended_input_shape, const integer_type* input_data, int32_t input_zero_point,
                 const RuntimeShape& unextended_output_shape, integer_type* output_data, int32_t output_zero_point) {
        ...
        for (int out_d = 0; out_d < output_depth; ++out_d) {
            ...
            for (int in_h = 0; in_h < input_height; ++in_h) {
                for (int in_w = 0; in_w < input_width; ++in_w) {
                    acc += input_data[Offset(input_shape, out_b, in_h, in_w, out_d)] - input_zero_point;
                }
            }
            ...
            output_data[Offset(output_shape, out_b, 0, 0, out_d)] = static_cast<integer_type>(acc);
        }
    }
}
