...
TfLiteStatus InitializeTfLiteTensorFromFlatbuffer(SimpleMemoryAllocator* allocator, bool allocate_temp,
...
class MicroAllocator {
  public:
    ...
    static MicroAllocator* Create(uint8_t* tensor_arena, size_t arena_size, ErrorReporter* error_reporter);
    ...
    static MicroAllocator* Create(SimpleMemoryAllocator* memory_allocator, ErrorReporter* error_reporter);
    ...
    TfLiteStatus StartModelAllocation(const Model* model, const MicroOpResolver& op_resolver,
                                      NodeAndRegistration** node_and_registrations, TfLiteEvalTensor** eval_tensors);
    ...
    TfLiteStatus FinishModelAllocation(const Model* model, TfLiteEvalTensor* eval_tensors,
    ...
    // buffers.
    virtual TfLiteTensor* AllocatePersistentTfLiteTensor(const Model* model, TfLiteEvalTensor* eval_tensors,
                                                         int tensor_index);
    ...
    virtual TfLiteTensor* AllocateTempTfLiteTensor(const Model* model, TfLiteEvalTensor* eval_tensors,
                                                   int tensor_index);
    ...
    TfLiteStatus RequestScratchBufferInArena(int node_id, size_t bytes, int* buffer_idx);
  ...
  protected:
    ...
    // model.
    virtual TfLiteStatus AllocateNodeAndRegistrations(const Model* model, NodeAndRegistration** node_and_registrations);
    ...
    // arena.
    virtual TfLiteStatus PrepareNodeAndRegistrationDataFromFlatbuffer(const Model* model,
    ...
    // eval_tensors.
    virtual TfLiteStatus AllocateTfLiteEvalTensors(const Model* model, TfLiteEvalTensor** eval_tensors);
    ...
    virtual TfLiteTensor* AllocatePersistentTfLiteTensorInternal(const Model* model, TfLiteEvalTensor* eval_tensors,
                                                                 int tensor_index);
    ...
    // the temp section.
    virtual TfLiteStatus PopulateTfLiteTensorFromFlatbuffer(const Model* model, const SubGraph* subgraph,
                                                            TfLiteTensor* tensor, int tensor_index, bool allocate_temp);
  ...
  private:
    ...
    virtual TfLiteStatus CommitStaticMemoryPlan(const Model* model, const SubGraph* subgraph,
