...
class MicroOpResolver : public OpResolver {
  public:
    typedef TfLiteStatus (*BuiltinParseFunction)(const Operator* op, ErrorReporter* error_reporter,
    ...
    const TfLiteRegistration* FindOp(BuiltinOperator op, int version) const final { return FindOp(op); }

    // This implementation exists for compatibility with the OpResolver base class
    // and disregards the version parameter.
    const TfLiteRegistration* FindOp(const char* op, int version) const final { return FindOp(op); }
