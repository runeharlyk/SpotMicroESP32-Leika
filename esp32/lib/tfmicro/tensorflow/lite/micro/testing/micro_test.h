...
#define TF_LITE_MICRO_TESTS_END                                                          \
    micro_test::reporter->Report("%d/%d tests passed", micro_test::tests_passed,         \
...
#define TF_LITE_MICRO_TEST(name)                                                                                 \
    ...
    for (micro_test::is_test_complete = false, micro_test::did_test_fail = false; !micro_test::is_test_complete; \
         micro_test::is_test_complete = true, micro_test::tests_passed += (micro_test::did_test_fail) ? 0 : 1,   \
...
#define TF_LITE_MICRO_EXPECT_EQ(x, y)                                                                    \
    do {                                                                                                 \
        ...
        auto vy = y;                                                                                     \
        if ((vx) != (vy)) {                                                                              \
            micro_test::reporter->Report(#x " == " #y " failed at %s:%d (%d vs %d)", __FILE__, __LINE__, \
...
#define TF_LITE_MICRO_EXPECT_NE(x, y)                                                          \
    do {                                                                                       \
        if ((x) == (y)) {                                                                      \
            micro_test::reporter->Report(#x " != " #y " failed at %s:%d", __FILE__, __LINE__); \
            micro_test::did_test_fail = true;                                                  \
        }                                                                                      \
    } while (false)

// TODO(wangtz): Making it more generic once needed.
#define TF_LITE_MICRO_ARRAY_ELEMENT_EXPECT_NEAR(arr1, idx1, arr2, idx2, epsilon)                                       \
    do {                                                                                                               \
        auto delta =                                                                                                   \
            ((arr1)[(idx1)] > (arr2)[(idx2)]) ? ((arr1)[(idx1)] - (arr2)[(idx2)]) : ((arr2)[(idx2)] - (arr1)[(idx1)]); \
        if (delta > epsilon) {                                                                                         \
            micro_test::reporter->Report(#arr1 "[%d] (%f) near " #arr2 "[%d] (%f) failed at %s:%d",                    \
                                         static_cast<int>(idx1), static_cast<float>((arr1)[(idx1)]),                   \
                                         static_cast<int>(idx2), static_cast<float>((arr2)[(idx2)]), __FILE__,         \
                                         __LINE__);                                                                    \
            micro_test::did_test_fail = true;                                                                          \
        }                                                                                                              \
    } while (false)
...
#define TF_LITE_MICRO_EXPECT_NEAR(x, y, epsilon)                                                               \
    do {                                                                                                       \
        ...
        if (delta > epsilon) {                                                                                 \
            micro_test::reporter->Report(#x " (%f) near " #y " (%f) failed at %s:%d", static_cast<double>(vx), \
...
#define TF_LITE_MICRO_EXPECT_TRUE(x)                                                              \
    do {                                                                                          \
        if (!(x)) {                                                                               \
            micro_test::reporter->Report(#x " was not true failed at %s:%d", __FILE__, __LINE__); \
            micro_test::did_test_fail = true;                                                     \
        }                                                                                         \
    } while (false)
...
#define TF_LITE_MICRO_EXPECT_FALSE(x)                                                              \
    do {                                                                                           \
        if (x) {                                                                                   \
            micro_test::reporter->Report(#x " was not false failed at %s:%d", __FILE__, __LINE__); \
            micro_test::did_test_fail = true;                                                      \
        }                                                                                          \
    } while (false)
    ...
    do {                                                                                                         \
        for (int i = 0; string1[i] != '\0' && string2[i] != '\0'; i++) {                                         \
            if (string1[i] != string2[i]) {                                                                      \
                micro_test::reporter->Report("FAIL: %s did not match %s", string1, string2, __FILE__, __LINE__); \
                micro_test::did_test_fail = true;                                                                \
            }                                                                                                    \
        }                                                                                                        \
    } while (false)
